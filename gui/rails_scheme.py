...
class RailsScheme:
    ...
    def _fill_branches(self):
        ...
        for branch in base.world.branches:  # noqa: F821
            start = -0.964 + self._world_map[branch["start"]].id * 0.002722
            ...
            end = -0.964 + self._world_map[branch["end"]].id * 0.002722
            self._temp_wids.append(
                DirectFrame(
                    parent=self._scheme,
                    frameTexture="gui/tex/dash.png",
    ...
    def _fill_scheme(self):
        ...
        outs = None
        cities = 0
        for block in self._world_map[:701]:
            if block.id % 100 == 0:
                if outs:
                    self._temp_wids.append(
                        DirectLabel(
                            ...
                            text_bg=(0, 0, 0, 0),
                            frameColor=(0, 0, 0, 0),
                            pos=(-0.964 + (block.id - 50) * 0.002722, 0, -0.1),
                        )
                    )
                outs = ""
            ...
            if block.is_city:
                self._temp_wids.append(
                    DirectFrame(
                        ...
                        frameTexture="gui/tex/city.png",
                        frameSize=(-0.04, 0.04, -0.04, 0.04),
                        pos=(-0.964 + block.id * 0.002722, 0, 0),
                    )
                )
                self._temp_wids.append(
                    DirectLabel(
                        ...
                        text_bg=(0, 0, 0, 0),
                        frameColor=(0, 0, 0, 0),
                        pos=(-0.964 + block.id * 0.002722, 0, 0.1),
                    )
                )
                cities += 1
        ...
        self._temp_wids.append(
            DirectFrame(
                ...
                frameSize=(
                    0,
                    base.world.stench_step * 0.002722,  # noqa: F821
                    -0.22,
                    0.22,
                ),
                pos=(-0.964, 0, 0),
